<?php
	...
	if ($local > $visitante){
	    $update_local = mysqli_query($conn, "UPDATE t_ligas SET pj = pj + 1, pg = pg + 1, pts = pts + 3, gf = gf + '$local', gc = gc + '$visitante' WHERE id_torneo = '$id_torneo' AND id_cuenta = '$enfval[1]'");
        $update_visitante = mysqli_query($conn, "UPDATE t_ligas SET pj = pj + 1, pp = pp + 1, gf = gf + '$visitante', gc = gc + '$local' WHERE id_torneo = '$id_torneo' AND id_cuenta = '$enfval[2]'");
        $update_enf = mysqli_query($conn, "UPDATE enfrentamientos SET tabla = '1' WHERE id_enfrentamiento = '$id_enf'");
	}
	if ($visitante > $local){
	    $update_local = mysqli_query($conn, "UPDATE t_ligas SET pj = pj + 1, pg = pg + 1, pts = pts + 3, gf = gf + '$visitante', gc = gc + '$local' WHERE id_torneo = '$id_torneo' AND id_cuenta = '$enfval[2]'");
        $update_visitante = mysqli_query($conn, "UPDATE t_ligas SET pj = pj + 1, pp = pp + 1, gf = gf + '$local', gc = gc + '$visitante' WHERE id_torneo = '$id_torneo' AND id_cuenta = '$enfval[1]'");
        $update_enf = mysqli_query($conn, "UPDATE enfrentamientos SET tabla = '1' WHERE id_enfrentamiento = '$id_enf'");
	}
	if ($visitante == $local){
	    $update_local = mysqli_query($conn, "UPDATE t_ligas SET pj = pj + 1, pe = pe + 1, pts = pts + 1, gf = gf + '$local', gc = gc + '$visitante' WHERE id_torneo = '$id_torneo' AND id_cuenta = '$enfval[1]'");
        $update_visitante = mysqli_query($conn, "UPDATE t_ligas SET pj = pj + 1, pe = pe + 1, pts = pts + 1, gf = gf + '$visitante', gc = gc + '$local' WHERE id_torneo = '$id_torneo' AND id_cuenta = '$enfval[2]'");
        $update_enf = mysqli_query($conn, "UPDATE enfrentamientos SET tabla = '1' WHERE id_enfrentamiento = '$id_enf'");
	}
	
	$update_goles = mysqli_query($conn, "UPDATE enfrentamientos SET gol_local = '$local', gol_visitante = '$visitante' WHERE id_enfrentamiento = '$id_enf'");
