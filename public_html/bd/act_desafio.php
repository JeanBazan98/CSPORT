<?php
	...
	if($desafiov[8] == 'Vuelta'){
	    if($enfv3[0] == $id_enf){
	        ...
	        if($enfv[0] == $id_enf){
        	    if ($gol_local + $enfv2[4] > $gol_visitante + $enfv2[3]) {
            		if(($enfv[10] == 1) || ($enfv2[10] == 1)){
            		    ...
            		    $ganano = mysqli_query($conn, "INSERT INTO wallet_t (id_cuenta, tipo, saldo, metodo, fecha, status) VALUES ('$enfv[1]','Desafio','$desafiov[9]','$id_torneo','$hoys','COMPLETED')");
                        $ganacp = mysqli_query($conn, "INSERT INTO wallet_t (id_cuenta, tipo, saldo, metodo, fecha, orden, status) VALUES ('0','Plataforma','$gncp','$id_torneo','$hoys','Desafio','COMPLETED')");
            		}
            	}
            	if ($gol_local + $enfv2[4] < $gol_visitante + $enfv2[3]) {
                    if(($enfv[10] == 1) || ($enfv2[10] == 1)){
                        $sqd2 = mysqli_query($conn, "UPDATE desafio SET ganador = '$enfv[2]' WHERE id_desafio = '$id_torneo'");
            		    $walletd = mysqli_query($conn, "UPDATE wallet SET saldo = saldo + '$desafiov[9]' WHERE id_cuenta = '$enfv[2]'");
            		    
            		    $ganano = mysqli_query($conn, "INSERT INTO wallet_t (id_cuenta, tipo, saldo, metodo, fecha, status) VALUES ('$enfv[2]','Desafio','$desafiov[9]','$id_torneo','$hoys','COMPLETED')");
                        $ganacp = mysqli_query($conn, "INSERT INTO wallet_t (id_cuenta, tipo, saldo, metodo, fecha, orden, status) VALUES ('0','Plataforma','$gncp','$id_torneo','$hoys','Desafio','COMPLETED')");
            		}
            	}
            	if ($gol_local + $enfv2[4] == $gol_visitante + $enfv2[3]) {
	    ...
	    if($enfv3[0] == $id_enf){
	        ...
	        if($enfv2[0] == $id_enf){
        	    if (($gol_local) > ($gol_visitante)) {
        		    ...
        		    $ganano = mysqli_query($conn, "INSERT INTO wallet_t (id_cuenta, tipo, saldo, metodo, fecha, status) VALUES ('$enfv2[1]','Desafio','$desafiov[9]','$id_torneo','$hoys','COMPLETED')");
                    $ganacp = mysqli_query($conn, "INSERT INTO wallet_t (id_cuenta, tipo, saldo, metodo, fecha, orden, status) VALUES ('0','Plataforma','$gncp','$id_torneo','$hoys','Desafio','COMPLETED')");
            	}
            	if (($gol_local) < ($gol_visitante)) {
                    $sqd2 = mysqli_query($conn, "UPDATE desafio SET ganador = '$enfv2[2]' WHERE id_desafio = '$id_torneo'");
        		    $walletd = mysqli_query($conn, "UPDATE wallet SET saldo = saldo + '$desafiov[9]' WHERE id_cuenta = '$enfv2[2]'");
        		    
        		    $ganano = mysqli_query($conn, "INSERT INTO wallet_t (id_cuenta, tipo, saldo, metodo, fecha, status) VALUES ('$enfv2[2]','Desafio','$desafiov[9]','$id_torneo','$hoys','COMPLETED')");
                    $ganacp = mysqli_query($conn, "INSERT INTO wallet_t (id_cuenta, tipo, saldo, metodo, fecha, orden, status) VALUES ('0','Plataforma','$gncp','$id_torneo','$hoys','Desafio','COMPLETED')");
            	}
            	if(($gol_local) == ($gol_visitante)) {
            	    $sql2dc1 = mysqli_query($conn, "INSERT INTO chat (id_torneo, id_cuenta, mensaje) VALUES ('$id_torneo','0','RESULTADO GLOBAL QUEDO EN EMPATE - SE DEFINE POR UN 3ER PARTIDO | POR GOL DE ORO')");
